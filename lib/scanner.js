...
function calculateRiskScore(checks) {
  ...
  // Homoglyph / karakter mirip (mis. tokoped1a.com, paypa1.com)
  // FIX: sebelumnya dihitung tapi tidak pernah memengaruhi skor
  if (!checks.isSafeDomain && checks.homoglyphs?.found?.length > 0) {
  ...
  // Safe domain bonus — bukan floor, tapi bonus yang wajar
  // Domain tepercaya dapat +15 tapi tetap bisa dikurangi faktor lain
  if (checks.isSafeDomain) {
    score = Math.min(100, score + 15);
  }

  // Variasi skor berdasarkan karakteristik URL — biar tidak semua 100
  // HTTPS tanpa path panjang = sedikit lebih tinggi
  if (checks.isHTTPS && !checks.hasLongPath) {
    score = Math.min(100, score + 3);
  }
  // Path sangat panjang atau banyak parameter — sedikit kurangi
  if (checks.hasLongPath) {
    score = Math.max(0, score - 3);
  }
  // URL sangat pendek dan bersih (contoh: google.com) = bonus kecil
