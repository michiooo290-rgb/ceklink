...
function analyzePath(pathname) {
  if (!pathname || pathname === "/") {
    return { suspicious: [], safe: true, description: "Path root — tidak ada halaman spesifik" };
  }
...
function detectDomainAgeHints(domain) {
  ...
  // Year patterns in domain (often newly created phishing domains)
  const yearMatch = lower.match(/20(2[0-9]|1[0-9])/);
  if (yearMatch) {
    indicators.push(`Mengandung tahun "${yearMatch[0]}" — domain phising sering pakai tahun`);
  }

  // "new", "baru", "latest" indicators
  ...
  // Very long random-looking domain (likely generated)
...
function detectRedirectHints(input) {
  ...
  // Common redirect patterns in URL
...
export async function scanURL(input) {
  ...
  const pathAnalysis = urlParts ? analyzePath(urlParts.pathname) : { suspicious: [], safe: true, segments: [], description: "" };
  const queryAnalysis = urlParts ? analyzeQueryParams(urlParts.search) : { suspicious: [], safe: [], total: 0, description: "" };
