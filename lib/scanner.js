 ...
 * - Fix: isSafeDomain diganti trust profile (official/user-controlled/unknown)
...
import {
  ...
  SUSPICIOUS_QUERY_PARAMS, DOMAIN_CONTEXT, USER_CONTROLLED_HOSTING,
} from "./scanner-data";
...
// ── Trust Profile ────────────────────────────────────────────────
/**
 * Klasifikasi domain berdasarkan tingkat kepercayaan:
 * - official_known: domain resmi platform yang dikenal (tokopedia.com, github.com)
 * - user_controlled_platform: domain hosting publik konten pengguna (github.io, vercel.app)
 * - managed_suffix: domain dengan suffix terkelola (.go.id, .ac.id)
 * - unknown: domain yang tidak dikenal
 */
function getTrustProfile(domain) {
  const clean = domain.replace(/^www\./, "").toLowerCase();

  // User-controlled hosting check (prioritas tinggi)
  for (const hosting of USER_CONTROLLED_HOSTING) {
    if (clean === hosting.domain || clean.endsWith("." + hosting.domain)) {
      return {
        type: "user_controlled_platform",
        hosting,
        label: `Platform hosting publik (${hosting.service})`,
  ...
  // Official known domain check
  const isSafeDomain = SAFE_DOMAINS.some(
    (safe) => clean === safe || clean.endsWith("." + safe)
  );
  if (isSafeDomain) {
    return {
      type: "official_known",
      label: "Domain/platform resmi yang dikenal",
      detail: "Domain ini dioperasikan oleh entitas resmi yang dikenal.",
      risk: "low",
    };
  }

  // Managed suffix check
  const managedSuffixes = [".go.id", ".ac.id", ".sch.id", ".gov", ".gov.uk", ".mil.id", ".or.id", ".polri.id", ".desa.id"];
  for (const suffix of managedSuffixes) {
    if (clean.endsWith(suffix)) {
      return {
        type: "managed_suffix",
        label: `Domain dengan suffix terkelola (${suffix})`,
        detail: `Domain dengan suffix ${suffix} dikelola oleh institusi, tetapi tetap perlu verifikasi.`,
        risk: "low",
      };
    }
  }
  ...
  return {
    type: "unknown",
    label: "Domain tidak dikenal",
    detail: "Domain ini tidak ada di database domain tepercaya kami.",
    risk: "medium",
  };
}

// ── User-Controlled Hosting Detection ───────────────────────────
function detectUserControlledHosting(domain) {
  const clean = domain.replace(/^www\./, "").toLowerCase();
  for (const hosting of USER_CONTROLLED_HOSTING) {
    if (clean === hosting.domain || clean.endsWith("." + hosting.domain)) {
      return hosting;
  ...
  return {
    ...
    details: {
      ...
      technicalDetails: {
        ...
        trustProfile: { type: "ip_address", label: "IP Address", detail: "Menggunakan IP langsung", risk: "high" },
        userHosting: null,
        isIPAddress: true,
...
function checkTyposquatting(domain) {
  ...
  // Kata kunci aksi yang menandakan domain phishing
  const actionWords = [
    "promo", "login", "update", "verifikasi", "secure", "bonus",
    "voucher", "claim", "hadiah", "diskon", "cashback", "auth",
    "oauth", "sso", "2fa", "security", "support", "recovery",
    "password", "otp", "pin", "akun", "rekening", "suspend",
    "blokir", "confirm", "undian", "prize", "winner", "reward",
  ];
  ...
  for (const brand of BRAND_KEYWORDS) {
    ...
    const patterns = [
      ...
      // Brand + action words
      ...actionWords.map((w) => cleanDomain.includes(b + w)),
    ...
    if (patterns.some(Boolean)) {
      // FIX: Untuk keyword pendek (≤3 char), tambahan validasi
      // Hindari false positive pada domain seperti "bri-llc.com" atau "bca-law.com"
      // yang memang mengandung brand tapi bukan phishing.
      // Wajib ada minimal 1 kata aksi phishing yang jelas, atau TLD mencurigakan.
      if (b.length <= 3) {
        const hasActionWord = actionWords.some((w) => cleanDomain.includes(w));
        const suspiciousTLD = /\.(xyz|top|buzz|click|link|work|racing|win|stream|gdn|bid|download|loan|men|party|science|trade|faith|date|review|country|cricket|gq|ml|cf|ga|tk|pw|cc|icu|cam|rest|surf|info|site|online|website|space|fun|store|shop|club|vip|pro|mobi)$/i.test(cleanDomain);
        const hasSubdomainTrick = cleanDomain.split(".").length > 3; // a.bri.evil.com

        if (!hasActionWord && !suspiciousTLD && !hasSubdomainTrick) continue;
      }
...
function calculateRiskScore(checks) {
  let score = 100;
  const deductions = [];
  let maxScoreCap = 100; // default: no cap

  const isOfficial = checks.trustProfile?.type === "official_known";
  const isUserControlled = checks.trustProfile?.type === "user_controlled_platform";
  const isKnown = isOfficial || isUserControlled;
  ...
  if (checks.typosquat) {
    ...
    const hasSuspiciousSuffix =
      /promo|sale|login|update|verify|bonus|voucher|claim|hadiah|diskon|cashback|auth|oauth|sso|2fa|security|support|recovery/.test(
        domainLower
      );
  ...
  // Keyword check — untuk domain unknown, penalty penuh; untuk known, penalty dikurangi
  if (checks.keywords.high.length > 0) {
    const kwPoints = isOfficial ? 5 : isKnown ? 10 : 15;
    score -= kwPoints;
    deductions.push({
      item: "Kata Kunci Phising di Domain",
      points: -kwPoints,
      reason: checks.keywords.high.join(", ") + (isOfficial ? " (dikurangi: domain resmi)" : ""),
    });
  }

  if (checks.keywords.medium.length > 0) {
    const kwPoints = isOfficial ? 3 : isKnown ? 7 : 10;
    score -= kwPoints;
    deductions.push({
      item: "Kata Kunci Mencurigakan di Domain",
      points: -kwPoints,
      reason: checks.keywords.medium.join(", ") + (isOfficial ? " (dikurangi: domain resmi)" : ""),
    });
  }
  ...
  // Homoglyph / karakter mirip
  if (checks.homoglyphs?.found?.length > 0 && !isOfficial) {
    let homoglyphPoints = 25;
    if (checks.homoglyphs.found.length >= 3) homoglyphPoints += 10;

    score -= homoglyphPoints;
    deductions.push({
      item: "Karakter Mirip (Homoglyph)",
      points: -homoglyphPoints,
      reason: `Ditemukan ${checks.homoglyphs.found.length} karakter yang menyamar (${checks.homoglyphs.found
        .map((h) => `'${h.char}'→'${h.looksLike}'`)
        .join(", ")}) — teknik umum typosquatting`,
    });
  }
  ...
  // ── Path Analysis ──
  if (checks.pathAnalysis?.suspicious?.length > 0) {
    const pathKeywords = checks.pathAnalysis.suspicious;
    // Credential paths lebih berbahaya
    const hasCredentialPath = pathKeywords.some((k) =>
      ["login", "signin", "sign-in", "password", "verify", "verifikasi", "otp", "pin", "2fa", "oauth", "auth", "sso"].includes(k)
    );
    const hasFileDownload = pathKeywords.some((k) =>
      ["download", "release"].includes(k)
    );

    // Untuk official domain, path login wajar; penalty hanya di non-official
    if (!isOfficial) {
      const pathPoints = hasCredentialPath ? (isUserControlled ? 20 : 12) : hasFileDownload ? 8 : 5;
      score -= pathPoints;
      deductions.push({
        item: "Path Mencurigakan",
        points: -pathPoints,
        reason: `Path mengandung: ${pathKeywords.join(", ")}`,
      });
    } else if (hasCredentialPath) {
      // Official domain dengan path credential — penalty sangat kecil
      score -= 2;
      deductions.push({
        item: "Path Kredensial di Domain Resmi",
        points: -2,
        reason: `Path ${pathKeywords.join(", ")} di domain resmi (normal)`,
      });
    }

    // User-controlled hosting + credential path = extra penalty
    if (isUserControlled && hasCredentialPath) {
      score -= 15;
      deductions.push({
        item: "Hosting Publik + Path Kredensial",
        points: -15,
        reason: `Platform hosting pengguna dengan path ${pathKeywords.join(", ")} — teknik phising umum`,
      });
      maxScoreCap = Math.min(maxScoreCap, 65);
    }

    // Suspicious file download di user-controlled hosting
    if (isUserControlled && hasFileDownload) {
      const hasSuspiciousExt = (checks.urlParts?.pathname || "").match(/\.(exe|apk|scr|msi|bat|cmd|ps1|sh|dmg|pkg|deb|rpm)$/i);
      if (hasSuspiciousExt) {
        score -= 15;
        deductions.push({
          item: "File Download Mencurigakan di Hosting Publik",
          points: -15,
          reason: `File ${hasSuspiciousExt[0]} di platform hosting pengguna`,
        });
        maxScoreCap = Math.min(maxScoreCap, 55);
      }
    }
  }

  // ── Query Parameter Analysis ──
  if (checks.queryAnalysis?.suspicious?.length > 0) {
    const queryCount = checks.queryAnalysis.suspicious.length;
    const hasRedirect = checks.queryAnalysis.suspicious.some((q) =>
      ["redirect", "redirect_url", "return", "return_url", "next", "goto", "dest", "destination", "target", "callback"].includes(q.key.toLowerCase())
    );

    // Cek apakah redirect value mengarah ke external URL
    let hasExternalRedirect = false;
    if (hasRedirect && checks.urlParts?.hostname) {
      for (const q of checks.queryAnalysis.suspicious) {
        try {
          const redirectUrl = new URL(q.value);
          if (redirectUrl.hostname !== checks.urlParts.hostname) {
    ...
    const queryPoints = isOfficial
      ? Math.min(queryCount * 2, 5)
      : hasExternalRedirect
      ? 15
      : hasRedirect
      ? 10
      : Math.min(queryCount * 5, 15);
    score -= queryPoints;
    deductions.push({
      item: "Parameter Mencurigakan",
      points: -queryPoints,
      reason: hasExternalRedirect
        ? "Parameter redirect mengarah ke URL eksternal"
        : hasRedirect
        ? "Parameter redirect terdeteksi"
        : `Parameter: ${checks.queryAnalysis.suspicious.map((q) => q.key).join(", ")}`,
    });
  }

  // ── Redirect Hints ──
  if (checks.redirectHints?.found?.length > 0) {
    const redirectCount = checks.redirectHints.found.length;
    const redirectPoints = isOfficial ? 2 : Math.min(redirectCount * 8, 20);
    score -= redirectPoints;
    deductions.push({
      item: "Indikator Redirect",
      points: -redirectPoints,
      reason: checks.redirectHints.found.join("; "),
    });
  }

  // ── Shannon Entropy ──
  if (checks.entropy?.level === "very_high") {
    const entPoints = isOfficial ? 0 : 8;
    if (entPoints > 0) {
      score -= entPoints;
      deductions.push({
        item: "Domain Sangat Acak",
        points: -entPoints,
        reason: `Entropy ${checks.entropy.score} — domain kemungkinan generated`,
      });
    }
  } else if (checks.entropy?.level === "high") {
    const entPoints = isOfficial ? 0 : 5;
    if (entPoints > 0) {
      score -= entPoints;
      deductions.push({
        item: "Domain Cukup Acak",
        points: -entPoints,
        reason: `Entropy ${checks.entropy.score} — perlu waspada`,
      });
    }
  }

  // ── Domain Age Hints ──
  if (checks.domainAgeHints?.likely_new && !isOfficial) {
    const agePoints = checks.domainAgeHints.indicators.length >= 3 ? 8 : 4;
    score -= agePoints;
    deductions.push({
      item: "Indikasi Domain Baru",
      points: -agePoints,
      reason: checks.domainAgeHints.indicators.join("; "),
    });
  }

  // ── User-Controlled Hosting ──
  if (isUserControlled && checks.userHosting) {
    const hosting = checks.userHosting;
    const hostPoints = hosting.risk === "high" ? 12 : 8;
    score -= hostPoints;
    deductions.push({
      item: "Platform Hosting Pengguna",
      points: -hostPoints,
      reason: `${hosting.service}: konten dikontrol oleh pengguna, bukan oleh platform`,
    });
    // Cap untuk user-controlled hosting yang clean
    if (!checks.typosquat && checks.keywords.high.length === 0) {
      maxScoreCap = Math.min(maxScoreCap, 80);
    }
  }
  ...
  const redFlags = [
    ...
    checks.homoglyphs?.found?.length > 0,
    isUserControlled,
  ...
  // Official domain bonus (lebih kecil dari sebelumnya)
  if (isOfficial) {
    score = Math.min(100, score + 10);
  }

  // Variasi skor berdasarkan karakteristik URL
  if (checks.isHTTPS && !checks.hasLongPath) {
    score = Math.min(100, score + 3);
  }
  if (checks.hasLongPath) {
    score = Math.max(0, score - 3);
  }
  if (checks.isCleanURL) {
  ...
  // Unknown domain cap: domain yang tidak dikenal dan bukan managed suffix
  // tidak boleh dapat skor > 85 karena belum terverifikasi aman
  const isUnknown = checks.trustProfile?.type === "unknown";
  if (isUnknown) {
    score = Math.min(score, 85);
  }

  // Final cap: never display 100, max is 95 for clean results
  score = Math.min(score, 95);

  return { score: Math.max(0, Math.min(95, score)), deductions, maxScoreCap };
}
...
export async function scanURL(input) {
  ...
  // Run all checks
  const trustProfile = getTrustProfile(domain);
  const isOfficial = trustProfile.type === "official_known";
  const isUserControlled = trustProfile.type === "user_controlled_platform";
  const userHosting = detectUserControlledHosting(domain);
  ...
  // Typosquatting: official domains skip false-positive brand match,
  // tapi tetap dicek untuk brand-in-deceptive-domain patterns
  const typosquat = checkTyposquatting(domain);
  const suspiciousTLD = isOfficial ? null : checkSuspiciousTLD(domain);
  const isShortURLResult = isShortURL(domain);

  // Keywords: penalty dikurangi untuk official, tapi tetap dicek
  const keywords = analyzeKeywords(domain);
  ...
  // Tambahan checks untuk variasi skor
  const pathname = urlParts?.pathname || "";
  const search = urlParts?.search || "";
  const hasLongPath = pathname.length > 60 || search.length > 80;
  const isCleanURL = pathname === "/" || pathname === "" || pathname.length < 10;
  const isHTTPS = hasSSL;

  const { score, deductions, maxScoreCap } = calculateRiskScore({
    hasSSL,
    typosquat,
    suspiciousTLD,
  ...
  if (isOfficial) {
    issues.push({
      ...
      value: "Domain/platform resmi yang dikenal",
      status: "safe",
      detail: "Domain ini dioperasikan oleh entitas resmi yang dikenal.",
    });
  }

  if (isUserControlled && userHosting) {
    issues.push({
      label: "Platform Hosting Pengguna",
      value: userHosting.service,
      status: userHosting.risk === "high" ? "warn" : "safe",
      detail: userHosting.detail,
    });
  }

  // Tambahkan issue "verifikasi eksternal belum selesai" nanti di UI
  // karena external check terjadi setelah return dari scanURL

  // Summary kontekstual — tidak generik, tidak overclaim
  let status, statusLabel, statusEmoji, riskLevel, summary;
  if (score >= 75) {
    status = "safe"; statusLabel = "RISIKO RENDAH"; statusEmoji = "✅"; riskLevel = "Rendah";
    const baseSummary = "Tidak ada indikator berbahaya yang terdeteksi dari pemeriksaan ini. Ini bukan jaminan aman; tetap verifikasi domain sebelum login, membayar, mengunduh file, atau memasukkan data.";
    if (isUserControlled && userHosting) {
      summary = `Domain berada di platform ${userHosting.service} yang kontennya dibuat oleh pengguna. ${baseSummary}`;
    } else if (domainContext) {
      summary = `${domainContext.name} adalah layanan ${domainContext.category.toLowerCase()} resmi. ${baseSummary}`;
    } else {
      summary = baseSummary;
    }
  } else if (score >= 55) {
  ...
  try {
    ...
    else threatCheckFailed = true;
  } catch {
    // Gagal hubungi server (offline, timeout, dst)
    threatCheckFailed = true;
  }

  // Tambahkan warning jika verifikasi database ancaman gagal
  if (threatCheckFailed) {
    issues.push({
      label: "Verifikasi Database Ancaman",
      value: "Tidak tersedia",
      status: "warn",
      detail: "Pemeriksaan ke URLhaus dan Google Safe Browsing gagal (timeout/error). Hasil hanya berdasarkan analisis pola heuristik — bisa saja URL berbahaya yang belum terdeteksi.",
    });
  }
    ...
    return {
      ...
      details: {
        ...
        technicalDetails: {
          ...
          trustProfile,
          userHosting: userHosting ? { service: userHosting.service, risk: userHosting.risk } : null,
          maxScoreCap,
        },
      },
      checkedAt: new Date().toISOString(),
